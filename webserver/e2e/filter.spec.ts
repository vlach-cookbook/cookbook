...
test("Recipe Filters", async ({ page, someFilterableRecipes }) => {
  ...
  await expect
    .soft(page.locator("#recipes").getByRole("listitem"))
    .toHaveText([sandwich.name, recipe1.name]);
});
...
test("Ingredient Filters", async ({ page, someFilterableRecipes }) => {
  ...
  await expect
    .soft(page.locator("#ingredients").getByRole("listitem").locator("summary"))
    .toHaveText([/^sugar$/i, /^eggs$/i, /^pickles$/i]);

  await page
    .locator("#ingredients")
    .getByRole("listitem")
    .filter({ hasText: "sugar" })
    .click();
  await expect
    .soft(
      page
        .locator("#ingredients")
        .getByRole("listitem")
        .filter({ hasText: /sugar/i })
        .getByRole("listitem")
    )
    .toHaveText([recipe1.name]);
  await expect
    .soft(
      page
        .locator("#ingredients")
        .getByRole("listitem")
        .filter({ hasText: /eggs/i })
        .getByRole("listitem")
    )
    .not.toBeVisible();
});
...
test("History Filters", async ({ page, someFilterableRecipes, testLogin }) => {
  ...
  const historyItems = page.locator("#history").getByRole("listitem");

  await expect
    .soft(historyItems.locator("summary"))
    .toHaveText(["Mar 2024", "Apr 2023"]);

  await page.getByLabel(/Filter/).fill("2024");
  await expect(page).toHaveURL("/history?filter=2024");

  await expect.soft(historyItems.locator("summary")).toHaveText(["Mar 2024"]);

  await expect
    .soft(historyItems.filter({ hasText: /Mar 2024/i }).getByRole("listitem"))
    .toHaveText([recipe1.name]);
});
...
test("Filter by user", async ({ page, testUser, testRecipe }) => {
  ...
  await expect
    .soft(page.getByRole("heading", { name: "Ingredients" }))
    .not.toBeVisible();
  await expect
    .soft(page.getByRole("heading", { name: "Categories" }))
    .not.toBeVisible();

  await expect.soft(page.getByRole("listitem")).toHaveText(["Recipe 2"]);
});

test("Back and Forward navigate through filters", async ({
  page,
  testUser,
  testRecipe,
}) => {
  ...
  await expect(
    page,
    "An empty filter should remove the query parameter."
  ).toHaveURL(/\/$/);
  await expect
    .soft(page.getByRole("listitem"))
    .toHaveText(["Ice Cream", "Sandwich"]);
...
test("Loading with a filter initializes single ingredients to open", async ({
  page,
  ...
  await expect(
    page
      .locator("#ingredients")
      .getByRole("listitem")
      .filter({ hasText: /butter/i })
      .getByRole("listitem")
      .filter({ hasText: recipe1.name })
  ).toBeVisible();
});

test("Loading with a filter initializes single categories to open", async ({
  page,
  testUser,
  testCategory,
  testRecipe,
